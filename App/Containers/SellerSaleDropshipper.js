...
class SellerSaleDropshipper extends React.Component {
  ...
  constructor (props) {
    ...
    this.state = {
      dataProduct: props.dataSalesDropship || null,
      saleList: [],
      page: 1,
      loadmore: false,
      isRefreshing: true,
  ...
  componentWillReceiveProps (nextProps) {
    const {dataSalesDropship} = nextProps

    if (!isFetching(dataSalesDropship) && this.submitting.product) {
      this.submitting = {
        ...this.submitting,
        product: false
      }
      if (isError(dataSalesDropship)) {
        ToastAndroid.show(dataSalesDropship.message, ToastAndroid.SHORT)
      }
      if (isFound(dataSalesDropship)) {
        const isFound = dataSalesDropship.sales.length
        this.setState({isRefreshing: false})
        if (isFound >= 10) {
          const data = [
            ...this.state.saleList,
            ...dataSalesDropship.sales
          ]
          this.setState({
            dataProduct: dataSalesDropship,
            isRefreshing: false,
            saleList: data,
            isLoading: false,
            loadmore: true,
...
const mapDispatchToProps = (dispatch) => ({
  getListSales: (param) => dispatch(salesAction.getSales2(param))
})

export default connect(mapStateToProps, mapDispatchToProps)(SellerSaleDropshipper)
